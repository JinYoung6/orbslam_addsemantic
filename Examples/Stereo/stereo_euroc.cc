...
void LoadImages(const string &strPathLeft, const string &strPathRight, const string &strPathTimes,
                vector<string> &vstrImageLeft, vector<string> &vstrImageRight, vector<long double> &vTimeStamps);
...
int main(int argc, char **argv)
{
    ...
    for(int ni=0; ni<nImages; ni++)
    {
        ...
        long double tframe = vTimeStamp[ni];//时间戳
        ...
        vTimesTrack[ni]=ttrack;//存储每一帧图像的跟踪时间
    ...
    SLAM.SaveKeyFrameTrajectoryEuroc("KeyFrameTrajectory2.txt");   
    return 0;
}
